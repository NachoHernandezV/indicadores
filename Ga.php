...
require_once 'jpgraph/src/jpgraph_pie.php';
	...
	///
			...
			if($Departamento[1]=="Sistemas")
		{
			...
			if($Indicador[1]=="Numero de Servicios")
			{
			$sql="SELECT m.mes,c.objetivo,c.reals from mes m, sis2_numserv c WHERE m.idmes=c.idmes ".$periodo." ORDER BY  m.idmes ASC";
			$nombreimagenfondo="sistemas1";
			}

			if($Indicador[1]=="Horas perdidas de red")
			{
			$sql="SELECT m.mes,c.objetivo,c.reals from mes m, sis3_horaslost c WHERE m.idmes=c.idmes ".$periodo." ORDER BY  m.idmes ASC";
			$nombreimagenfondo="sistemas2";
			}

			if($Indicador[1]=="Desarrollo de software")
			{
			$sql="SELECT m.mes,c.objetivo,c.reals from mes m, sis4_desarrollo c WHERE m.idmes=c.idmes ".$periodo." ORDER BY  m.idmes ASC";
			$nombreimagenfondo="sistemas3";
			}
		}
