...
//obtener EL NOMBRE DE DEPARTAMENTO Y EL INDICADOR
   ...
   $_SESSION['meses']=$mesnum;
		...
		if($nombredepartamento[1]=="Sistemas")
	{
		...
		{
			$q="SELECT m.mes,c.objetivo,c.reals from mes m, sis1_efic c WHERE m.idmes=c.idmes ".$periodo." ORDER BY  m.idmes ASC";
			$tabla1 = mysqli_query($link,$q);
			$extraido = mysqli_fetch_row($tabla1);
		}
		...
		{
			$q="SELECT m.mes,c.objetivo,c.reals from mes m, sis2_numserv c WHERE m.idmes=c.idmes ".$periodo." ORDER BY  m.idmes ASC";
			$tabla1 = mysqli_query($link,$q);
			$extraido = mysqli_fetch_row($tabla1);
		}
		...
		{
			$q="SELECT m.mes,c.objetivo,c.reals from mes m, sis3_horaslost c WHERE m.idmes=c.idmes ".$periodo." ORDER BY  m.idmes ASC";
			$tabla1 = mysqli_query($link,$q);
			$extraido = mysqli_fetch_row($tabla1);
		}
		...
		{
			$q="SELECT m.mes,c.objetivo,c.reals from mes m, sis4_desarrollo c WHERE m.idmes=c.idmes ".$periodo." ORDER BY  m.idmes ASC";
			$tabla1 = mysqli_query($link,$q);
			$extraido = mysqli_fetch_row($tabla1);
		}
	}
...
if($Guardar == "Guardar")
{
		...
		if($nombredepartamento1=="Sistemas")
	{
		...
		{
				$q = "UPDATE sis2_horas SET objetivo='$user', reals='$realz', nota='$comentarios' WHERE year1 ='".$year1."' and idmes='". $_SESSION['meses']."'";
				$tabla1 = mysqli_query($link,$q);
		}
		if($nombreindicador1=="Eficienciac de red")
		{
				$q = "UPDATE sis3_red SET objetivo='$user', reals='$realz', nota='$comentarios' WHERE year1 ='".$year1."' and idmes='". $_SESSION['meses']."'";
				$tabla1 = mysqli_query($link,$q);
		}
	}
